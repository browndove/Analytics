...
// Total: 11 + 21 + 16 + 4 + 12 + 6 = 70
const barData: BarData[] = [
    { month: "JUN", value: 11, maxValue: 25 },
    { month: "MAY", value: 21, maxValue: 25 },
    { month: "APR", value: 16, maxValue: 25 },
    { month: "MAR", value: 4, maxValue: 25 },
    { month: "FEB", value: 12, maxValue: 25 },
    { month: "JAN", value: 6, maxValue: 25 },
];

const NearMissIncidents: React.FC = () => {
    const [isMaximized, setIsMaximized] = React.useState(false);
    const [animatedBars, setAnimatedBars] = React.useState(barData.map(() => 0));
    const [animatedTotal, setAnimatedTotal] = React.useState(0);
    const [isVisible, setIsVisible] = React.useState(false);

    const totalIncidents = 70;
    ...
    React.useEffect(() => {
        ...
        const animate = () => {
            ...
            setAnimatedBars(barData.map(item => (item.value / item.maxValue) * 100 * eased));
            setAnimatedTotal(Math.round(totalIncidents * eased));

            if (progress < 1) {
                requestAnimationFrame(animate);
            } else {
                setAnimatedBars(barData.map(item => (item.value / item.maxValue) * 100));
                setAnimatedTotal(totalIncidents);
            }
        };
        requestAnimationFrame(animate);
    }, [isVisible]);
        ...
        <>
            <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-[10px]">
                    <div className="bg-[#2980D31A] px-[7px] py-[4px] rounded-[5px]">
            ...
            <div className={`flex flex-col gap-[21px] ${isModal ? "flex-1" : "flex-1"}`}>
                {barData.map((item, index) => (
                    <div key={index} className="flex items-center gap-[10px]">
                        <Text variant="body-xs" color="text-tertiary" className="w-[30px]">
                            {item.month}
                        </Text>
                        <div className="flex-1 h-[25px] bg-tertiary rounded-[5px] overflow-hidden">
                            <div
    ...
    return (
        <>
            <DashboardCard className="flex flex-col gap-[15px] h-full" padding="lg">
                {chartContent(false)}
            </DashboardCard>
...
export default NearMissIncidents;
