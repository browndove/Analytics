...
interface TopCallingDepartmentProps {
    callMetrics?: CallMetricsSlice;
}

const TopCallingDepartment: React.FC<TopCallingDepartmentProps> = ({ callMetrics }) => {
    const top = getTopDepartment(callMetrics);
    const count = top ? num(top.total_calls_made) : null;
    const deptLabel = top ? truncateLabel(top.department_name, 28) : null;
...
export default TopCallingDepartment;
