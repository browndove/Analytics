...
const infoText =
    ...
    'Bar = escalation rate index for that role (normalized across the three roles shown). ' +
            ...
            <div className="mt-3 flex flex-col gap-3">
                ...
                {topRoles.map((role, index) => {
                    ...
                    return (
                        ...
                        >
                            <div className="flex items-center justify-between gap-2">
                                <p className="min-w-0 flex-1 truncate text-[12px] leading-tight text-text-primary">
                                ...
                                <div
                                    className={clsx(
                                        `${style.bgColor} shrink-0 rounded-full px-2.5 py-0.5`,
                                        'transition-transform duration-300',
                                        isHovered && 'scale-105'
                                    )}
                                >
                                    <Text variant="body-xs-semibold" color={style.textColor} className="whitespace-nowrap">
                                        {badgeRate}% index
                                    </Text>
                                </div>
                            </div>
                            {detailParts.length > 0 && (
                                <Text variant="body-xs" color="text-secondary" className="leading-tight line-clamp-2">
                                    {detailParts.join(' · ')}
                                </Text>
                            )}

                            <div className={`${style.bgColor} mt-0.5 h-[5px] rounded-full w-full overflow-hidden`}>
                                <div
