...
const infoText = "Diagnostic imaging volume by modality including CT scans, MRI, X-Ray, and Ultrasound.";
...
const ImagingRadiology: React.FC = () => {
    const [isFullscreen, setIsFullscreen] = useState(false);
    const { resolvedTheme } = useTheme();
    const [isHovered, setIsHovered] = useState(false);
    const [animatedCompleted, setAnimatedCompleted] = useState(0);
    const [animatedQueue, setAnimatedQueue] = useState(0);
    const [isVisible, setIsVisible] = useState(false);

    const scansCompleted = 230;
    const inQueue = 180;
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            setAnimatedCompleted(Math.round(scansCompleted * eased));
            setAnimatedQueue(Math.round(inQueue * eased));

            if (progress < 1) {
                requestAnimationFrame(animate);
            } else {
                setAnimatedCompleted(scansCompleted);
                setAnimatedQueue(inQueue);
            }
        };
        requestAnimationFrame(animate);
    }, [isVisible]);
    ...
    const chartOptions: ApexCharts.ApexOptions = {
        ...
        colors: ["var(--accent-red)", "var(--accent-green)"],
        plotOptions: {
            bar: {
                horizontal: false,
                columnWidth: "40px",
        ...
        xaxis: {
            categories: ["CT Scan", "MRI", "X-Ray", "Ultrasound"],
    ...
    useEffect(() => {
        // Small delay to ensure chart is mounted before data is set, forcing animation
...
interface ChartStatsProps {
    ...
    animatedQueue: number;
}

const ChartStats = ({ animatedCompleted, animatedQueue }: ChartStatsProps) => (
